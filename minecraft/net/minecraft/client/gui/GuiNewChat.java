...
public class GuiNewChat extends Gui
{
    ...
    private float percentComplete, animationPercent;
    private int newLines, lineBeingDrawn;
    ...
    private void updatePercentage(float speed, long diff) {
        if (this.isScrolled || !hudModule.getElementsOption().find("Chat Animation").getValue()) this.percentComplete = 1;
        else if (this.percentComplete < 1) {
            this.percentComplete += speed * diff;
            this.percentComplete = MathUtil.INSTANCE.clamp(this.percentComplete, 0, 1);
        }
    }

    public void drawChat(int updateCounter)
    {
        this.hudModule = (HUDModule) Main.moduleManager.getModuleByClass(HUDModule.class);
        assert this.hudModule != null;
        if (this.mc.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN)
        {
            ...
            if (k > 0)
            {
                ...
                this.updatePercentage((float) (this.hudModule.getChatAnimationSpeedOption().getValue() / 10), diff);
                float t = percentComplete;
                this.animationPercent = MathUtil.INSTANCE.clamp(1 - (--t) * t * t * t, 0, 1);

                float f1 = this.getChatScale();
                int l = MathHelper.ceiling_float_int((float)this.getChatWidth() / f1);
                GlStateManager.pushMatrix();
                if (hudModule.getState() && hudModule.getElementsOption().find("Chat Animation").getValue())
                ...
                for (int i1 = 0; i1 + this.scrollPos < this.drawnChatLines.size() && i1 < i; ++i1)
                {
                    ...
                    this.lineBeingDrawn = i1 + this.scrollPos;
                    ...
                    if (chatline != null)
                    {
                        ...
                        if (j1 < 200 || flag)
                        {
                            ...
                            if (l1 > 3)
                            {
                                int i2 = 0;
                                int j2 = -i1 * 9;
                                boolean viewable = hudModule.getElementsOption().find("Chat Animation").getValue() && this.lineBeingDrawn <= this.newLines;
                                if (viewable && !flag) {
                                    if (!hudModule.getElementsOption().find("Fast Chat").getValue()) {
                                        drawRect(i2, j2 - 9, i2 + l + 4, j2, new Color(0F, 0F, 0F, this.animationPercent * ((float)d0 / 2F)).getRGB());
                                    }
                                } else {
                                    if (!hudModule.getElementsOption().find("Fast Chat").getValue()) {
                                ...
                                if (viewable) {
                                    this.mc.fontRendererObj.drawStringWithShadow(s, (float)i2, (float)(j2 - 8), new Color(1F, 1F, 1F, animationPercent * (float)d0).getRGB());
                                }else {
