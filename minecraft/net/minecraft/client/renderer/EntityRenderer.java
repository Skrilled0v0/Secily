...
import me.kl0udy92.apart.features.module.implementations.attack.ReachModule;
import me.kl0udy92.apart.features.module.implementations.visual.CameraNoClipModule;
import me.kl0udy92.apart.features.module.implementations.world.WeatherModule;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        {
            final ReachModule reach = (ReachModule) Main.moduleManager.getModuleByClass(ReachModule.class);
            ...
            if (this.mc.playerController.extendedReach())
            {
                ...
                if (reach.getChanceOption().getValue() == 100.0) {
                    d0 = d1 = reach.getBlockRangeOption().getValue();
                } else if (Math.random() >= reach.getChanceOption().getValue() / 100) {
                    d0 = d1 = reach.getBlockRangeOption().getValue();
                }
            } else if (d0 > 3.0D)
            {
            ...
            if (reach.getState()) {
                if (reach.getChanceOption().getValue() == 100.0) {
                    d1 = reach.getRangeOption().getValue();
                } else if (Math.random() >= reach.getChanceOption().getValue() / 100) {
                    d1 = reach.getRangeOption().getValue();
                }
            }
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (movingobjectposition != null)
                    {
                        ...
                        if (d7 < d3 && !Main.moduleManager.getModuleByClass(CameraNoClipModule.class).getState())
                        {
                            d3 = d7;
                        }
                    }
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        EventManager.INSTANCE.fire(new Render3DEvent(partialTicks));
            ...
            for (int l = 0; l < k; ++l)
            {
                ...
                if (blockpos1.getY() <= blockpos.getY() + i && blockpos1.getY() >= blockpos.getY() - i && biomegenbase.canRain() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F)
                {
                    ...
                    else if (block.getMaterial() != Material.air)
                    {
                        ...
                        if (Main.moduleManager.getModuleByClass(WeatherModule.class) != null && Main.moduleManager.getModuleByClass(WeatherModule.class).getState()) {
                            if (!(Objects.equals(((WeatherModule) (Main.moduleManager.getModuleByClass(WeatherModule.class))).getModeOption().getValue(), "Snowy"))) {
                            ...
                            if (world.getWorldChunkManager().getTemperatureAtHeight(f1, j2) >= 0.15F)
                            {
                                if (j1 != 0)
                                {
                                    ...
                                    if (Main.moduleManager.getModuleByClass(WeatherModule.class) != null && Main.moduleManager.getModuleByClass(WeatherModule.class).getState()) {
                                        ...
                                        }else this.mc.getTextureManager().bindTexture(locationRainPng);
                                    }else this.mc.getTextureManager().bindTexture(locationRainPng);
