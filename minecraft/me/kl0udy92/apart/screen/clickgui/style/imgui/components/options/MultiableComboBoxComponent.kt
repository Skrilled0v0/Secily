...
@NativeClass
class MultiableComboBoxComponent(
    ...
    override fun render(mouseX: Int, mouseY: Int) {
        ...
        Main.fontBuffer.SF16.drawString(
            if (this.multiBooleanOption.value.any { it.value })
                "${this.multiBooleanOption.value.filter { it.value }.size} Selected."
            else "......",
            this.x + 12.0,
        ...
        if (this.extended) {
            RenderUtil.drawRect(
                ...
                this.x + 3.0 + this.width - 7 - 4.5,
                this.daddy.daddy.gui.y + this.daddy.y + this.y - this.daddy.daddy.scrollTranslate.y + 28.0 + (this.multiBooleanOption.value.size * 14.0),
                Palette.LINE_GRAY.color.rgb
            )
            RenderUtil.drawRect(
                this.x + 8.0,
                ...
                this.daddy.daddy.gui.y + this.daddy.y + this.y - this.daddy.daddy.scrollTranslate.y + 27.5 + (this.multiBooleanOption.value.size * 14.0),
                Palette.BACKGROUND.color.rgb
            )
            this.multiBooleanOption.value.forEach {
                val hovering = MouseUtil.isHovered(
                    this.x + 12.0,
                    yAxis.get(),
                    Main.fontBuffer.SF16.getStringWidth(it.name).toDouble(),
    ...
    override fun update(mouseX: Int, mouseY: Int) {
        if (this.extended) {
            this.offset = 30.0 + (this.multiBooleanOption.value.size * 14.0) + 5.0
    ...
    override fun mouseClicked(mouseX: Int, mouseY: Int, button: Int) {
        when (button) {
            MouseUtil.ButtonType.LEFT.identifier -> {
                if (this.extended && MouseUtil.isHovering(
